...
func (y *YTDLP) DownloadLocal(ctx context.Context, rawURL, formatID, outputTemplate string, progress func(float64)) error {
	...
	for scanner.Scan() {
		match := progressPercentPattern.FindStringSubmatch(scanner.Text())
		if len(match) != 2 || progress == nil {
			continue
		}
	...
	if err := cmd.Wait(); err != nil {
		...
		log.Warn().Err(err).Str("stderr", message).Msg("local yt-dlp download failed")
